...
class NativeCartAddProduct extends Simulation{
  val writer = {
  ...
  var sentHeaders = Map("v" -> "5.0.1","Content-Type" -> "application/json","os" -> "ios","os_version" -> "9.3.4")  
   val httpProtocol = http
	...
	val scn = scenario("NativeCartAddProduct")
	.exec(http("request_1")
...
"""))
  .check(header("Login-Token").saveAs("token") 
	...
	.pause(2)
	.exec(http("request_0")
	.post("/service/wishlist/addProduct")
	.headers(Map("v" -> "5.0.1","Content-Type" -> "application/json","os" -> "ios","os_version" -> "9.3.4","Login-Token" -> "${token}"))
	.body(StringBody("""
       {
	"requestProtocol": "PROTOCOL_JSON",
	"responseProtocol": "PROTOCOL_JSON",
	"pogIds": ["684758524171", "654131425823", "642057816655"],
	"collectionName": "Snapdeal"
}
        """)).asJSON
   .check(status.is(200))
