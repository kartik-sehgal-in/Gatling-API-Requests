...
class LoginLinkEmailToMobileOnlyAccount extends Simulation{
 val writer = {
  ...
  var sentHeaders = Map("v" -> "5.0.1","Content-Type" -> "application/json","os" -> "ios","os_version" -> "9.3.4")  
   val httpProtocol = http
	...
	val scn = scenario("LoginLinkEmailToMobileOnlyAccount")
	.exec(http("request_1")
...
"""))
  .check(header("Login-Token").saveAs("token") 
	...
	.pause(2)
	.exec(http("request_0")
	.post("/service/user/linkEmailToMobileOnlyAccount")
